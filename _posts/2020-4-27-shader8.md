...
本文内容主要参考 《Unity shader入门精要》一书，旨在总结所学知识与加深个人理解，为了表示方便，与书中公式略有差别。
...
$\vec r$是反射方向，反射可以通过计算获得 $\vec r=2(\vec r\cdot\vec l)\vec n-\vec l$    
#### Blinn模型
公式 
$$
\mathtt{c}_{specular}=(\mathtt{c}_{light}\cdot\mathtt{m}_{specular})max(0,\vec v\cdot\vec h)^{m_{gloss}}
$$     
