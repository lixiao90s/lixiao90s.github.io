...
我们在shader中对法线纹理采样后，需要进行反映射 $normal=pixel\ast 2-1$。
...
而z轴是顶点的法线方向（n）,x轴是顶点的切线方向（t）,而y轴可由法线和切线叉积而得，也被称为副切线。   
...
- 可进行UV动画。我们可以移动一个纹理的UV坐标来实现凹凸移动的效果，但使用模型空间下的法线纹理得到完全错误的结果。这种UV动画在水
或者火山熔岩。
- 可以重用法线纹理。
- 可压缩。    
...
定义了TANGENT_SPACE_ROTATION

### 实现
_BumpMap,使用"bump"作为它的默认值。"bump"是Unity内置法线纹理，当没有提供任何法线纹理时，bump对应模型自带法线信息。
